...
{{ if .Values.redis.authentication.username }}
...
{{ if .Values.redis.prefix.enabled }}
key = {{ quote .Values.redis.prefix.value }}..key
{{ end }}

local data, err = red:setex(key, expirationTime, value)
if not data then
